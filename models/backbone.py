...
try:
    ...
    local_ckpt = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'pretrained', 'efficient_x3d_xs_original_form.pyth'))
    if os.path.exists(local_ckpt):
        checkpoint = torch.load(local_ckpt, map_location='cpu')
        model_efficient_x3d_xs.load_state_dict(checkpoint, strict=False)
    else:
        checkpoint = load_state_dict_from_url(checkpoint_path, map_location='cpu')
        model_efficient_x3d_xs.load_state_dict(checkpoint, strict=False)
except Exception as e:
    # Fall back to randomly initialized EfficientX3D if download/load fails
    print(f"[Warning] Failed to load EfficientX3D weights ({e}). Proceeding without pretrained weights.")
