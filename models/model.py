...
class DroneGuard(nn.Module):
    ...
    def __init__(self, config, pretrained=True):
        ...
        attn_device = "cuda" if torch.cuda.is_available() else "cpu"
        self.attn = MultiheadGQA(embed_dim=embedding_dim, query_heads=4, kv_heads=2, device=attn_device)
        ...
        # Quantization Layer
        self.vq = Quantizer(channels[3], codebook_size=128, num_quantizers=4, Quantizer_name='VectorQuantize')
    ...
    def forward(self, x):
        ...
        x = self.up8(x8)
        # Ensure spatial alignment before concatenation
        ...
        if x0.shape[-2:] != x.shape[-2:]:
            x0 = F.interpolate(x0, size=x.shape[-2:], mode='bilinear', align_corners=False)
