...
def main():
    ...
    # Build loss functions on the selected device
    ...
    if(args.pseudo):
        ...
        train_loader_jump = torch.utils.data.DataLoader(
            train_dataset_jump,
            batch_size=config.TRAIN.BATCH_SIZE_PER_GPU * (len(gpus) if use_cuda else 1),
    ...
    if(args.test):
        ...
        test_loader = torch.utils.data.DataLoader(
            test_dataset,
            batch_size=BATCH_SIZE_PER_GPU_TEST * (len(gpus) if use_cuda else 1),
            shuffle=False,
            num_workers=config.WORKERS,
            pin_memory=True
        )
    ...
    for epoch in range(last_epoch, config.TRAIN.END_EPOCH):
        ...
        if (epoch + 1) % config.SAVE_CHECKPOINT_FREQ == 0:
            ...
            torch.save(state, os.path.join(final_output_dir, 'epoch_{}.pth'.format(epoch + 1)))
        
        if (epoch % 10 ==0) and ( args.output_folder != None):
            state = model.module.state_dict() if isinstance(model, nn.DataParallel) else model.state_dict()
            torch.save(state, os.path.join(args.output_folder, 'epoch_train_{}.pth'.format(epoch + 1)))
        if(args.test)and(epoch > 15):
            ...
            if(auc>AUC_memory):
                ...
                state = model.module.state_dict() if isinstance(model, nn.DataParallel) else model.state_dict()
                torch.save(state, os.path.join(final_output_dir, 'best_AUC.pth'.format(epoch + 1)))
                if( args.output_folder != None):
                    torch.save(state, os.path.join(args.output_folder, 'best_AUC.pth'.format(epoch + 1)))                                               
                AUC_memory=auc
    ...
    state = model.module.state_dict() if isinstance(model, nn.DataParallel) else model.state_dict()
    torch.save(state, final_model_state_file)
...
def train(config, train_loader, model, loss_functions, optimizer, epoch, logger,args):
    ...
    for i, data in enumerate(train_loader):
        ...
        # Support both DataParallel and single model
        model_run = model.module if isinstance(model, nn.DataParallel) else model
        output,loss_commit = model_run.compute_loss(inputs)
...
def train_pseudo(config, train_loader,train_loader_jump, model, loss_functions, optimizer, epoch, logger,pseudolossepoch, pseudolosscounter,args):
    ...
    for i, (data,data_jump) in enumerate(zip(train_loader,train_loader_jump)):
       ...
       # target_batch_jump=To_Batch(target_jump,config.TRAIN.BATCH_SIZE_PER_GPU * len(list(config.GPUS)),config.MODEL.ENCODED_FRAMES)
        ...
        inputs = [input.to(device) for input in inputs]
        model_run = model.module if isinstance(model, nn.DataParallel) else model
        output,loss_commit_ = model_run.compute_loss(inputs)
